...
import type { AppLevelRewardClaimableItem, KycRewardCurrency } from '@/api/types';
...
type Props = {
  visible: boolean;
  submitting?: boolean;
  item: AppLevelRewardClaimableItem | null;
  onClose: () => void;
  onConfirm: (currency: KycRewardCurrency) => void;
};

export function LevelRewardModal({ visible, submitting, item, onClose, onConfirm }: Props) {
  ...
  const hint =
    item.claimPolicy === 'ALL'
      ? `${item.levelName}等级奖励，请选择到账币种（可分别领取）`
      : `${item.levelName}等级奖励，请选择到账币种（仅可领取一种）`;
    ...
    <Modal transparent visible={mounted} animationType="none" onRequestClose={onClose}>
      <View style={styles.root}>
        ...
        <Animated.View style={[styles.card, { transform: [{ scale }] }]}>
          ...
          <Text style={styles.title}>领取等级奖励</Text>
          ...
          <View style={styles.options}>
            {item.options.map((option) => (
              <Option
                key={option.currency}
                label={formatKycRewardLabel(option.currency, option.amount)}
                sub={option.currency === 'CNY' ? '人民币到账' : 'USDT 到账'}
                selected={selected === option.currency}
                onPress={() => setSelected(option.currency)}
              />
            ))}
          </View>
