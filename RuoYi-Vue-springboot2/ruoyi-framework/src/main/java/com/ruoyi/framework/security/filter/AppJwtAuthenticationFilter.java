...
public class AppJwtAuthenticationFilter extends OncePerRequestFilter
{
            ...
            throws ServletException, IOException
    {
        ...
        if (uri.startsWith("/app/") && !uri.startsWith("/app/auth/"))
        {
            AppLoginMember loginMember = appTokenService.getLoginMember(request);
            if (StringUtils.isNotNull(loginMember) && StringUtils.isNull(SecurityUtils.getAuthentication()))
            {
                appTokenService.verifyToken(loginMember);
